...
def train_one_epoch(train_dataloader, model, optimizer, loss_function):
    """
...
def valid_one_epoch(valid_dataloader, model, loss_function):
    """
...
def optimize(data_loaders, model, optimizer, loss, n_epochs, save_path, interactive_tracking=False):
    ...
    scheduler  = StepLR(optimizer, step_size=30, gamma=0.5, verbose=False)
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if valid_loss_min is None or (
            ...
            filename = save_path + "best_val_loss_v" + f'{best_val_version:0>2}' + ".pt"
            torch.save(model.state_dict(), filename)
            best_val_version += 1

            valid_loss_min = valid_loss

        # For every five epochs, save the model
        if epoch % 5 == 0:
            print(f"Saving model ... epoch {epoch:0>2}")
            filename = save_path + "/epoch_{:0>2}_{:0>2}".format((epoch - 4), epoch) + ".pt"
            torch.save(model.state_dict(), filename)
...
def one_epoch_test(test_dataloader, model, loss_function):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data, target) in tqdm(
            ...
            logits  = model(data)
            # 2. calculate the loss
            loss_value  = loss_function(logits, target).detach()
